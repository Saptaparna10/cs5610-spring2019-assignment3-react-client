...
const Widget = ({widget, preview, disableUp, disableDown, dispatch}) => {
    ...
    return (
        ...
        <li className="" style={{listStyle: 'none', border: '1px dotted grey', padding: 20}}>
            ...
            <div>
                {widget.type === 'HEADING' && <HeadingContainer widget={widget}/>}
                {widget.type === 'PARAGRAPH' && <ParagraphContainer widget={widget}/>}
                {widget.type === 'LIST' && <ListContainer widget={widget}/>}
                {widget.type === 'IMAGE' && <ImageContainer widget={widget}/>}
                {widget.type === 'LINK' && <LinkContainer widget={widget}/>}
            </div>
        </li>
    )
}
